...
void HadesInterface::ReadEvent() {
  ...
  // int(fEvtChara.getCentralityEstimator(fCEmapBK.at(i).second),
  // fConfig.GetBranchConfig(fEventHeader->GetId()).GetFieldId(
  // fCEmapNames.second ) ) ); 	fEventHeader->SetField(
  // float(fEvtChara.getCentralityPercentile(fCEmapBK.at(i).second),
  // fConfig.GetBranchConfig(fRecEventHeader->GetId()).GetFieldId("centrality_"+fCEmapNames.second)));
  // }
}
...
void HadesInterface::ReadFw() {
  ...
  const int iModule_id =
      fConfig.GetBranchConfig(fFwHits->GetId()).GetFieldId("module_id");
  const int iBeta =
      fConfig.GetBranchConfig(fFwHits->GetId()).GetFieldId("beta");
  const int iRing =
      fConfig.GetBranchConfig(fFwHits->GetId()).GetFieldId("ring");
  const int iTime =
      fConfig.GetBranchConfig(fFwHits->GetId()).GetFieldId("time");
...
void HadesInterface::ReadVertexTracks() {
  ...
  const int iPhi =
      fConfig.GetBranchConfig(fVtxTracks->GetId()).GetFieldId("phi");
